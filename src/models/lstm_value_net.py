...
class LSTMValue(DeterministicMixin, Model):
    ...
    def compute(self, inputs, role):
        ...
        # Handle case where batch_size < sequence_length
        if batch_size < self.sequence_length:
            pad_size = self.sequence_length - batch_size
            padding = torch.zeros(
                pad_size, states.shape[-1], device=states.device, dtype=states.dtype
            )
            states_padded = torch.cat([states, padding], dim=0)
            rnn_input = states_padded.unsqueeze(0)
        else:
